...
> **最后更新**: 2026-06-22 — P0/P1 全部完成；P2 已完成 A07/A10/A11/A12+A24/A13/A16/A17/A19/A20/A21/A22/A23/A27，下一个研究项目为 A26
...
│   │   └── ...7个辅助文件
│   ├── ShelfView/
│   │   ├── RecentShelfView.swift           # SwiftUI 最近书架
│   │   ├── SectionShelfView.swift          # SwiftUI 浏览书架
│   │   ├── ShelfBookCard.swift             # SwiftUI 书卡组件
│   │   └── ShelfDisplayModels.swift        # 纯值类型显示模型
│   ├── SettingsView/
...
| 指标 | 数值 |
|------|------|
| **总 Swift 代码行数** | **36,558 行** (含测试与拆分后文件) |
| **Swift 源文件数** | **217 个** (含测试 target) |
| **超过 300 行的文件** | **15 个** (占总行数 60%+) |
| **超过 1000 行的文件** | **5 个** (占总行数 32%) |
| **已标记废弃的代码项** | **11 处** (~600 行) |
| **导入 RealmSwift 的文件** | **50 个** (含视图层 18 个；schema/mapper 拆分增加文件数) |
...
### [A12] RealmModel.swift — 1388 行的 Realm 模型巨文件 ✅ 已完成 (P2/A12+A24, 2026-06-19)

- **位置**：~~[RealmModel.swift](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Models/RealmModel.swift)~~ → 已拆分到 `Models/Realm/`
- **现象**：12+ 个 Realm Object 子类全部定义在单个文件中。包含废弃的 `CalibreBookLastReadPositionRealm`。复杂迁移逻辑与 Readium preference mapping 混在持久化层。
- **状态**：✅ 已拆分为聚焦的 Realm schema 文件；`FolioReaderPreferenceRealm` 移入模型/持久化层；`ReadiumPreferenceRealm` 与 `EPUBPreferences`/`PDFPreferences`/settings 的转换已移到 `Views/ReadiumView/ReadiumPreferenceAdapter.swift`，核心 `Models/` 层不再导入 `ReadiumNavigator`/`ReadiumShared`。
- **严重程度**：🟡 中 → ✅ 已解决
...
### [A16] BookDetailView — 802 行的超大 SwiftUI 视图 ✅ 已完成 (P2/A16, 2026-06-22)
...
- **状态**：✅ 已分三阶段完成：拆出 `BookDetailContentView`/`BookDetailToolbar`，将原 `BookDetailSubviews.swift` 拆为 `BookCoverView`、`BookMetadataSection`、`BookProgressSection`、`BookConnectivitySection`、`BookFormatList`、`BookFormatRow`、`BookCountPagesCorner`，并把进度摘要、下载查询、格式展示与预览 sheet 状态集中到 `BookDetailViewModel`。最新完整验证：131 unit + 1 UI。
- **严重程度**：🟡 中 → ✅ 已解决

---

### [A17] LibraryInfoBookListView — 714 行，业务逻辑内联 ✅ 已完成 (P2/A17, 2026-06-22)
...
- **状态**：✅ 已在 commit `27af6b1` 完成 MVVM 拆分：新增 `LibraryInfoBookListViewModel`，拆出 `LibraryInfoBookListHeader`、`LibraryInfoBookListFooter`、`LibraryInfoBookListToolbar`、`LibraryInfoBookListContent`、`LibraryInfoBookContextMenu`，并新增 `LibraryInfoBookListViewModelTests`。提交统计：10 files changed, 870 insertions, 383 deletions。
- **严重程度**：🟡 中 → ✅ 已解决
...
### [A19] 网络层无统一错误处理 ✅ 已完成 (P2/A19, 2026-06-20)
...
- **状态**：✅ 已分 A19-S1...A19-S8 完成 remediation：统一使用 `CalibreAPIError` 与 `validatedData`/publisher 边界，修复 category/search/metadata/helper/upload 的错误吞没或降级，保留必要 legacy adapter，并清理视图与日志边界。最新完整验证：122 unit + 1 UI。
- **严重程度**：🟡 中 → ✅ 已解决
...
### [A21] UIKit/SwiftUI 混合书架 — 复杂桥接 ✅ 已完成 (P2/A21, 2026-06-22)

- **位置**：
  - SwiftUI 原生实现：`RecentShelfView.swift` 和 `SectionShelfView.swift`
  - 遗留的 UIKit 文件：`RecentShelfController.swift`, `RecentShelfUI.swift`, `SectionShelfController.swift`, `SectionShelfUI.swift` 已完全删除。
...
- **当前状态**：✅ 已完成。已通过 6 阶段迁移并去除了 UIKit 书架控制器和 `ShelfView` SPM 依赖，完全原生 SwiftUI 化。
- **严重程度**：🟡 中 → ✅ 已解决
...
### [A24] Readium 类型泄漏到 RealmModel 持久化层 ✅ 已完成 (P2/A12+A24, 2026-06-19)

- **位置**：~~[RealmModel.swift](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Models/RealmModel.swift)~~ → mapping 移至 [ReadiumPreferenceAdapter.swift](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Views/ReadiumView/ReadiumPreferenceAdapter.swift)
- **现象**：Realm 模型文件依赖 Readium SDK 类型（如 `Locator`）进行阅读位置序列化。
- **影响**：升级 Readium SDK 时可能需要 Realm 数据迁移。持久化层与阅读引擎耦合，无法独立演进。
- **状态**：✅ Readium preference/settings 转换已移出持久化模型层，`Models/` 不再直接依赖 Readium Navigator/Shared 类型。
- **严重程度**：🟡 中 → ✅ 已解决
...
- **位置**：
- **当前代码位置**：
  - [YabrReadiumReaderViewController.swift:L603](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Views/ReadiumView/YabrReadiumReaderViewController.swift#L603): 音量键事件处理后固定 0.1 秒重置 baseline 并解锁 `isHandlingVolumeChange`。
  - [YabrReadiumReaderViewController.swift:L655](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Views/ReadiumView/YabrReadiumReaderViewController.swift#L655): 固定 0.1 秒递归等待 `MPVolumeView` 内部 `UISlider` 初始化。
- **现象**：Readium 音量键翻页依赖固定延迟处理 AVAudioSession/MPVolumeView/KVO 时序，而非明确的事件完成或布局 readiness。
- **影响**：在不同性能的设备、音频会话状态、前后台切换或 MPVolumeView 初始化速度下行为不一致。可能导致翻页跳过、重复触发或 volume-key paging 初始化失败。
- **当前状态**：🟡 已完成研究并选为下一执行项。方案见 `.agents/memory-bank/p2-a26-readium-volume-key-timing-plan.md`。
- **严重程度**：🟢 低

---

### [A27] Realm ↔ 值类型手动转换 — 大量样板代码 ✅ 已完成 (P2/A27, 2026-06-22)

- **位置**：原先散布在 Realm schema、Repository、Manager、Service 和 ViewModel 中；现集中到 `Models/Realm/CalibreRealmMappers.swift`、`ReadingPositionRealmMappers.swift`、`AnnotationRealmMappers.swift`。
- **现象**：每个 Realm Object 都有对应的 struct 在 CalibreData.swift 中，转换需要约 50 行手写映射代码。转换逻辑分散在多个文件中。
- **影响**：新增字段需要在 3-4 个地方同步修改。转换代码容易遗漏字段。
- **状态**：✅ 已完成。Commit `1e053da` 引入显式 mapper、`makeRealmObject`/`applyDomain` 写侧 helper、Repository 调用点迁移、远程 annotation payload 边界清理，并新增 `RealmDomainMappingTests`。最新验证记录：174 unit tests passed。
- **严重程度**：🟡 中 → ✅ 已解决
...
| **P1** | A18 | 完成 V2 迁移 | ✅ 完成 |
| **P1** | A09 | 重构 DSReaderHelper 线程安全 | ✅ 完成 |
| **P2** | A10 | 移动 ShelfDataManager 到 Models | ✅ 完成 |
| **P2** | A11 | 拆分 CalibreData.swift | ✅ 完成 (1→10 files) |
| **P2** | A07+A13+A22 | 清理废弃代码 | ✅ 完成 |
| **P2** | A12+A24 | 拆分 RealmModel + 解耦 Readium | ✅ 完成 |
| **P2** | A16 | 拆分 BookDetailView | ✅ 完成 |
| **P2** | A17 | 拆分 LibraryInfoBookListView | ✅ 完成 |
| **P2** | A19 | 统一网络错误处理 | ✅ 完成 |
| **P2** | A21 | 书架视图 SwiftUI 原生化 | ✅ 已完成 |
| **P2** | A27 | Realm ↔ 值类型转换样板收敛 | ✅ 已完成 |
| **P2** | A26 | Readium timing hack 清理 | 🟡 下一执行项 |

> **P0 定义**：不改会阻塞后续所有重构 — ✅ 全部完成  
> **P1 定义**：高价值，可独立完成 — ✅ 全部完成
> **P2 定义**：有益但可延后 — 主要高风险项与 A21 已完成
...
```
剩余 ─── P1 收尾 + P2 延后任务 ──────────────────────────────
├── [P2-A26] Readium timing hack 清理（已研究，待执行）
...
graph LR
    ...
    P1_VM --> P2_Shelf["✅ 书架原生化"]
    P2_Data --> P2_Mapping["✅ Realm/value 映射"]
    P1_Reader --> P2_ReadiumTiming["🟡 Readium timing"]
...
> **关键路径更新**：P0 ✅ → P1 ✅ → P2 收尾；A27 已完成，A26 Readium timing hack 为下一执行项。
> 
> 每个阶段完成后通过 `xcodebuild build` + `xcodebuild test` 验证。当前 A27 后记录基线为 174 unit。
