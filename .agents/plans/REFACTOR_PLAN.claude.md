...
> **最后更新**: 2026-06-22 — P0/P1 全部完成；P2 已完成 A07/A10/A11/A12+A24/A13/A16/A17/A19/A20/A22/A23，下一个研究项目为 A21
...
- **位置**：
  - [SectionShelfController.swift](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Views/ShelfView/SectionShelfController.swift) (543行) — UIKit `SectionShelfCompositionalView` controller
  - [RecentShelfController.swift](file:///Users/peterlee/git/YetAnotherEBookReader/YetAnotherEBookReader/Views/ShelfView/RecentShelfController.swift) (474行) — UIKit `PlainShelfView` controller
...
- **当前决策**：📝 已选为下一个研究项目。`RecentShelfViewModel` 与 `SectionShelfViewModel` 已存在，`ShelfDataManager` 已移动到 `Models/`，具备渐进迁移条件。方案见 `.agents/memory-bank/p2-a21-shelf-swiftui-native-plan.md`。
- **严重程度**：🟡 中
...
| **P2** | A19 | 统一网络错误处理 | ✅ 完成 |
| **P2** | A21 | 书架视图 SwiftUI 原生化 | 📝 下一项目 |

> **P0 定义**：不改会阻塞后续所有重构 — ✅ 全部完成  
> **P1 定义**：高价值，可独立完成 — ✅ 全部完成
> **P2 定义**：有益但可延后 — 主要高风险项已完成；A21 为下一项
...
```
剩余 ─── P1 收尾 + P2 延后任务 ──────────────────────────────
├── [P2-A21] 书架视图 SwiftUI 原生化 (~3天，下一项目)
...
> **关键路径更新**：P0 ✅ → P1 ✅ → P2 收尾；A21 是下一个可独立推进的现代化项目
> 
> 每个阶段完成后通过 `xcodebuild build` + `xcodebuild test` 验证。当前完整基线为 131 unit + 1 UI；A17 已在 commit `27af6b1` 完成并提交。
